...
namespace ContextMenuCustomApp.View.Menu
{
    public class MenuPageViewModel : BaseViewModel
    {
        ...
        public MenuItem New()
        {
            var item = new MenuItem() { Title = "My context menu", Param = @"""{path}""", AcceptFile = true, AcceptDirectory = true };
            MenuItems.Add(item);
            return item;
        }
        ...
        public async Task BuildCache()
        {
            await RunWith(async () =>
            {
                ...
                OnMessage("Cache built successfully");
            });
        }
        ...
        public async void SetCustomMenuName(string name)
        {
            await Task.Run(() =>
            {
                ApplicationData.Current.LocalSettings.Values["Custom_Menu_Name"] = name ?? "Custom";
            });
        }
